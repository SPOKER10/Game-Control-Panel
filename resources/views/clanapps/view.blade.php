...
@section('breadcrumb',$app->typeText . ' / View')
		...
		<div class="row">
			...
			<div class="panel panel-default">
				...
				<div class="list-group">
					...
					<div class="list-group-item app-details-children"><strong>Type:</strong><span class="app-type"> {!! $app->typeText !!} </span></div>
					<div class="list-group-item app-details-children"><strong>Status:</strong><span class="app-status"> {!! $app->statusText !!} </span></div>
					@if($app->status != 0)<div class="list-group-item app-details-children"><strong>Other specifications:</strong><span class="app-status"> {{ $app->specifications }} </span></div>@endif
				</div>
				@if(Auth::user()->ID == $app->userid) <form method="post" action=""><button class="btn btn-danger btn-sm btn-block" name="app_cancel" @if(in_array($app->status,[1,2,3,4])) disabled="disabled" @endif><i class="fa fa-lock"></i> Cancel Application</button>{{ csrf_field() }}</form> @endif
				@if((Auth::user()->AdminLevel >= 5) || (Auth::user()->Clan+1 == $app->clan && Auth::user()->CRank > 5))
					<div class="panel-footer clearfix">
						<form method="post" action="">
							@if($app->status < 2)<textarea name="appc" rows="1" placeholder="Aici poti scrie cateva cuvinte pentru jucator..." class="form-control">{{ old('appc') }}</textarea></br>@endif
							<div class="col-xs-6 col-button-left">
								@if($app->status == 0 && !$app->type) <?php $method = 'app_tests'; ?> @else <?php $method = 'app_accept'; ?> @endif
								<button class="btn btn-success btn-sm btn-block" name="{{ $method }}" @if(in_array($app->status,[2,3,4])) disabled="disabled" @endif>
									<i class="fa fa-check"></i> @if($app->status == 0 && !$app->type) Accept for tests @else Accept @endif 
								</button>
							</div>
							<div class="col-xs-6 col-button-right">
								<button class="btn btn-danger btn-sm btn-block" name="app_reject" @if(in_array($app->status,[2,3,4])) disabled="disabled" @endif>
